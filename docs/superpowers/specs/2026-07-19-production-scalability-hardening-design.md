...
- **Cloudflare Pages SPA fallback**: a `public/_redirects` file with `/*  /index.html  200` so client-side-routed deep links (e.g. a scanned `/coffesarowar/durbarmarg/claim?token=…`) resolve to the SPA shell instead of a 404 — the production-host equivalent of the Vite dev server's history fallback, which Cloudflare Pages needs explicitly.
